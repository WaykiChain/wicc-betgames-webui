<template>
  <div class="dice-home">
      ...
      <div class="show-content">
        <div class="content" :style="loopStyle">
          <div class="line" v-for="item in loopData" :key="item.id">
            ...
            <span class="reward" v-if="lang == 'en' &&item.operateType == 200">{{$t('dice.home[0]')}} {{item.fee}} COIN {{$t('dice.home[1]')}} {{item.displayPeriodNum}}</span>
            <span class="reward" v-if="lang == 'zh-CHS' &&item.operateType == 200 ||lang == 'zh-CHT' &&item.operateType == 200">在{{item.displayPeriodNum}}{{$t('dice.home[0]')}}{{item.fee}} COIN</span>
            <span class="reward" v-if="lang == 'en' &&item.operateType == 100">{{$t('dice.home[2]')}} {{item.fee}} COIN {{$t('dice.home[1]')}} {{item.displayPeriodNum}}</span>
            <span class="reward" v-if="lang == 'zh-CHS' &&item.operateType == 100 || lang == 'zh-CHT' &&item.operateType == 100">在{{item.displayPeriodNum}}{{$t('dice.home[1]')}}{{item.fee}} COIN</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="main" id="main">
      ...
      <div style="overflow: hidden;" :style="{height:recordHeight+'px'}">
        <div v-if="stop" class="stop-toast">{{$t('dice.stop[0]')}}</div>
        <table id="data-table" :style="{transform:tableTopT}" v-else>
          <tbody>
        ...
        <div class="bet" id="bet" v-show="activeLine=='line1'">
          ...
          <div class="info2">
            <div>
              ...
              <span class="num">{{proBonus}}COIN</span>
            </div>
            <div>
          ...
          <v-touch class="sure-bet" @touchstart.native="betStart" @touchend.native="betEnd" :style="{backgroundImage:'url('+betBk+')'}">
            <span v-if="stop" style="color:red;">{{$t('dice.home_tab1[6]')}}</span>
            <span v-else >{{$t('dice.home_tab1[5]')}}</span>
          </v-touch>
        </div>
          ...
          <div class="table-head" id="table-head">
            <table>
                <tr style="height:auto;">
                  ...
                  <th style="width:26%;">{{$t('dice.home_tab2[3]')}}(COIN)</th>
                </tr>
            </table>
          </div>
        ...
        <div class="my-order" id="my-order" v-show="activeLine=='line3'" v-infinite-scroll="loadMore1" infinite-scroll-disabled="busy1" infinite-scroll-distance="10">
          ...
          <div class="order-line" v-for="item in orderData" :key="item.id" >
            <div class="flex">
              ...
              <span class="title large" v-if="item.status == 200" style="color:#70FF4D;">+{{item.bonus}}COIN</span>
            </div>
            <div class="flex">
              <span class="title">{{$t('dice.home_tab3[3]')}}: {{item.displayPeriodNum +5}}</span>
              <span class="title">{{$t('dice.home_tab3[4]')}}: {{item.betFee}}COIN</span>
            </div>    
            <div class="flex">
...
<script>
// @ is an alias to /src
let bet = 'https://abc.com/games/assets/dice/image/bet-sure.png'
let betOn = 'https://abc.com/games/assets/dice/image/bet-sure-active.png'
let check = 'https://abc.com/games/assets/dice/image/check-hash.png'
let checkOn = 'https://abc.com/games/assets/dice/image/check-hash-on.png'
let music = 'https://abc.com/games/assets/dice/image/music.png'
let musicS = 'https://abc.com/games/assets/dice/image/music-stop.png'
...
export default {
  ...
  mounted() {
    ...
    self.$nextTick(function() {
      ...
      self.recordHeight = document.getElementById('main').offsetHeight;
    })
  },
  activated() {
    this.addScript();
  ...
  methods:{
    ...
    goBack(){
      document.getElementById("dice-bk").pause()
      
      bridge.callhandler("notifyAppPop",{id:123456}, function(responseData) {
    ...
    changeTab(id) {
      ...
      if (id =='line2') {
        this.$buryPoint("clickTab", "掷骰子", "点击获奖投注tab")
        this.getRecordData();
        this.orderPage = 1;
        this.orderData = [];
        this.orderDataLoading = true;
      } else if (id=='line3') {
        this.$buryPoint("clickTab", "掷骰子", "点击我的投注tab")
        this.recordPage =1;
        this.recordData = [];
        this.getOrderData();
      } else {
        this.$buryPoint("clickTab", "掷骰子", "点击下注tab")
        this.recordPage =1;
        this.recordData = [];
        this.orderPage = 1;
        this.orderData = [];
    ...
    changeSection(id) {
      ...
      if (id == 2) {
        this.$buryPoint("clickOpt", "掷骰子", "点击等于")
      } else if (id == 1) {
        this.$buryPoint("clickOpt", "掷骰子", "点击小于")
      } else if (id == 3) {
        this.$buryPoint("clickOpt", "掷骰子", "点击大于")
      }
      this.setPro();
    },
    changeSize(id) {
      ...
      if(id == '1/2') {
        ...
        this.$buryPoint("clickSize", "掷骰子", "点击1/2")
      } else if(id == '2X') {
        this.$buryPoint("clickSize", "掷骰子", "点击2x")
        ...
        this.$buryPoint("clickSize", "掷骰子", "点击MAX")
        // TODO 取最大余额
    ...
    betEnd() {
      if(this.stop) return;
      this.$buryPoint("betEnd", "掷骰子_点击下注", "点击下注")
      this.betBk = bet;
      if (this.beting) {
        return;
      }
      ...
      .then(function (response) {
        ...
        if(response.code == 0){
          ...
          self.$buryPoint("diceBet", "掷骰子_下注成功", "下注成功")
        } else {
          let tost;
          if (self.lang == 'en') {
            tost = 'Bet failure'
    ...
    toRule() {
      this.$buryPoint("clickRule", "掷骰子", "点击查看规则")
    ...
    setBordUp() {
      const top = document.getElementById('input').top;
      this.$buryPoint("clickInput", "掷骰子", "点击下注值输入框")
      window.scroll(0,500);
    },
    setKeyBord() {
      window.scroll(0,0);
