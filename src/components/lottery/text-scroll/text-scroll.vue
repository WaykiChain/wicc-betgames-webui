<template>
  ...
  <transition-group tag="ul" :name="scrollType" class="text-scroll">
    <li v-for="(item, index) in textList" :key="`random${index}`" v-show="index==count">
    <template v-if="langs != 'en'">
      {{item.userName}}<span>在</span>{{item.issueNo}}<span>{{item.option == 1 ? $t('lottery.history_title18[0]') : $t('lottery.history_title18[1]')}}</span>{{item.amount | retainedDecimal}}<span>个</span>COIN
    </template>
    <template v-else>
      {{item.userName}} {{item.option == 1 ? $t('lottery.history_title18[0]') : $t('lottery.history_title18[2]')}} {{item.amount | retainedDecimal}} COIN {{item.option == 1 ? $t('lottery.history_title18[1]') : $t('lottery.history_title18[3]')}} {{item.issueNo}}
    </template>
    <template>
